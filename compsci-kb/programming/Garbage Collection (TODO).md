



## Algorithms and Theory
- [Mark-and-Sweep: Garbage Collection Algorithm - GeeksforGeeks](https://www.geeksforgeeks.org/mark-and-sweep-garbage-collection-algorithm/)
- [Static single-assignment form - Wikipedia](https://en.wikipedia.org/wiki/Static_single-assignment_form)

## Modern Day implementations


### V8 (Node JS, Chromium browsers)
- [Trash talk: the Orinoco garbage collector · V8](https://v8.dev/blog/trash-talk)
- [High-performance garbage collection for C++ · V8](https://v8.dev/blog/high-performance-cpp-gc)
- [Garbage collection in V8, an illustrated guide | by Irina Shestak | Medium](https://medium.com/@_lrlna/garbage-collection-in-v8-an-illustrated-guide-d24a952ee3b8)
- [smol-zines/guides/garbage-collection-in-v8.md at main · lrlna/smol-zines (github.com)](https://github.com/lrlna/smol-zines/blob/main/guides/garbage-collection-in-v8.md#-sourcesjs)
- [v8-perf/gc.md at master · thlorenz/v8-perf (github.com)](https://github.com/thlorenz/v8-perf/blob/master/gc.md)
- [JavaScript garbage collection: Browser vs. server - LogRocket Blog](https://blog.logrocket.com/javascript-garbage-collection-browser-vs-server/)
- [JavaScript Internals: Garbage Collection | AppSignal Blog](https://blog.appsignal.com/2020/10/21/garbage-collection-in-javascript.html)
- [Mastering Memory Management in Node.js: Tips and Tricks for Better Performance (youtube.com)](https://www.youtube.com/watch?v=YBnN2JpS4hI)
- [Understand JS Garbage Collector in 4 mins (youtube.com)](https://www.youtube.com/watch?v=FZ42HMWG6xg)
### Python
- [Python Garbage Collection: What It Is and How It Works (stackify.com)](https://stackify.com/python-garbage-collection/)
- [Garbage Collection in Python - GeeksforGeeks](https://www.geeksforgeeks.org/garbage-collection-python/)
- [gc — Garbage Collector interface — Python 3.12.0 documentation](https://docs.python.org/3/library/gc.html)